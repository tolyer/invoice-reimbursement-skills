...
一套面向 WorkBuddy / Codex 的双阶段差旅报销工作流：先在本地整理、核验并归档凭证，再把确认后的结果录入用友云（YonBIP）发票池。

仓库包含两个彼此衔接、边界清晰的 skill：
...
> 这不是通用财务系统，也不是“全自动报销”。两个 skill 都采用先预览、后确认的两步式流程，把不可逆的平台操作留给用户明确授权之后。
